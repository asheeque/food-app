...
const CUISINE_TYPES = ['Arabic / Middle Eastern', 'Indian', 'Pakistani', 'Filipino', 'Chinese', 'Western / Continental', 'Fast Food', 'Bakery / Café', 'Seafood', 'Mixed']
const DELIVERY_ZONES = ['Deira', 'Bur Dubai', 'Downtown / Business Bay', 'Jumeirah', 'Al Quoz', 'Sharjah', 'Other']
...
export default function RestaurantSignupPage() {
  ...
  const [form, setForm] = useState({
    restaurantName: '', brandGroup: '', contactName: '',
    whatsapp: '', email: '', password: '',
    cuisineType: '', deliveryZone: '', preferredTime: '',
  })
    ...
    try {
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({
          ...
          brandGroup:    form.brandGroup || null,
        }),
      })
      ...
      <div className="max-w-2xl mx-auto px-6 py-12">
        <div className="mb-10">
          ...
          <p className="text-sm" style={{ color: '#6B7280' }}>
            Create your account and start placing orders directly from the portal.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
          ...
          <div className="rounded-xl p-7 bg-white" style={{ border: '1px solid #E5E7EB' }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label htmlFor="restaurantName" className="block text-sm font-semibold mb-1.5" style={{ color: '#111827' }}>
                  Restaurant name <span style={{ color: '#C9943E' }}>*</span>
                </label>
                <input id="restaurantName" name="restaurantName" type="text" placeholder="Al Safadi Restaurant" value={form.restaurantName} onChange={handleChange} required className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors" style={inputStyle} {...focusHandlers} />
              </div>
              <div>
                <label htmlFor="brandGroup" className="block text-sm font-semibold mb-1.5" style={{ color: '#111827' }}>
                  Brand / Group <span className="text-xs font-normal" style={{ color: '#6B7280' }}>(optional)</span>
                </label>
                <input id="brandGroup" name="brandGroup" type="text" placeholder="Hospitality Group LLC" value={form.brandGroup} onChange={handleChange} className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors" style={inputStyle} {...focusHandlers} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="cuisineType" className="block text-sm font-semibold mb-1.5" style={{ color: '#111827' }}>Cuisine type</label>
                  <select id="cuisineType" name="cuisineType" value={form.cuisineType} onChange={handleChange} className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors appearance-none" style={inputStyle} {...focusHandlers}>
                    <option value="">Select…</option>
                    {CUISINE_TYPES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label htmlFor="deliveryZone" className="block text-sm font-semibold mb-1.5" style={{ color: '#111827' }}>Delivery zone</label>
                  <select id="deliveryZone" name="deliveryZone" value={form.deliveryZone} onChange={handleChange} className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors appearance-none" style={inputStyle} {...focusHandlers}>
                    <option value="">Select…</option>
              ...
              <div>
                <label htmlFor="preferredTime" className="block text-sm font-semibold mb-1.5" style={{ color: '#111827' }}>
                  Preferred delivery time <span className="text-xs font-normal" style={{ color: '#6B7280' }}>(e.g. 6:00 AM – 9:00 AM)</span>
                </label>
                <input id="preferredTime" name="preferredTime" type="text" placeholder="6:00 AM – 9:00 AM" value={form.preferredTime} onChange={handleChange} className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors" style={inputStyle} {...focusHandlers} />
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl p-7 bg-white" style={{ border: '1px solid #E5E7EB' }}>
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <input id="contactName" name="contactName" type="text" placeholder="Ahmed Hassan" value={form.contactName} onChange={handleChange} required autoComplete="name" className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors" style={inputStyle} {...focusHandlers} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <input id="email" name="email" type="email" placeholder="manager@restaurant.ae" value={form.email} onChange={handleChange} required autoComplete="email" className="w-full px-4 py-3 rounded-lg text-sm outline-none transition-colors" style={inputStyle} {...focusHandlers} />
                </div>
              </div>
              <div>
